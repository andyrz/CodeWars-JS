...
// their length and when they have the same length sorted in ascending lexicographic order (letters and digits - more precisely 
...
var string1 = " In many languages";
var string2 = " there's a pair of functions";

function mix(s1, s2){

    var workingArray1 = s1.split("");
    var workingArray2 = s2.split("");
    ...
    var map1 = workingArray1.reduce((elements, item) => {
        if(re.exec(item)){
            if (!elements.get(item)) {
                elements.set(item, 1);
            } else {
    ...
    var map2 = workingArray2.reduce((elements, item) => {
        if(re.exec(item)){
            if (!elements.get(item)) {
                elements.set(item, 1);
            } else {
    ...
    for (const [key, value] of map1.entries()) {
        ...
        if(value > 1 || map2.get(key) > 1){
            if(value < map2.get(key)){
               ...
               map2.delete(key);
               deleted = true;
               returnArray.push(s2Separator + temp);            
            }
            if(value > map2.get(key)){
                temp = "";
                ...
                map2.delete(key);
                deleted = true;
                returnArray.push(s1Separator + temp);
            }
            if(value === map2.get(key)){
                temp = "";
                for(let i=0; i < value; i++){
                ...
                map2.delete(key);
                deleted = true;
                returnArray.push(equalsColon + temp);
            }               
        }
        if(value > 1 && !map2.get(key) && !deleted){
                temp = "";
                for(let i=0; i < value; i++){
                    temp = temp + key;
                }
    ...
    returnArray.sort(function(a, b){
      ...
      return returnArray.join(forwardSlash);
}
